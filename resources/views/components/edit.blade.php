...
@section('title', 'Edit ' . $component->name)
@section('heading', 'Edit Component')

@section('content')
    <div class="max-w-2xl">
        <form method="POST" action="{{ route('components.update', $component) }}" class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <select id="owner_id" name="owner_id"
                    ...
                    @foreach ($activeUsers as $user)
                        <option value="{{ $user->id }}" @selected(old('owner_id', $component->owner_id) == $user->id)>{{ $user->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $component->name) }}" required
            ...
            <div>
                ...
                <select id="lifecycle_stage" name="lifecycle_stage"
                    ...
                    @foreach ($lifecycleStages as $stage)
                        <option value="{{ $stage->value }}" @selected(old('lifecycle_stage', $component->lifecycle_stage?->value) === $stage->value)>{{ $stage->value }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label for="lifecycle_start_date" class="block text-sm font-medium text-gray-700">Lifecycle Start Date</label>
                        @if (old('lifecycle_start_date', $component->lifecycle_start_date))
                            <button type="button" onclick="document.getElementById('lifecycle_start_date').value = ''"
                                    class="text-xs text-gray-400 hover:text-gray-600">Clear</button>
                        @endif
                    </div>
                    <input type="date" id="lifecycle_start_date" name="lifecycle_start_date"
                           value="{{ old('lifecycle_start_date', $component->lifecycle_start_date?->toDateString()) }}"
                ...
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label for="lifecycle_end_date" class="block text-sm font-medium text-gray-700">Lifecycle End Date</label>
                        @if (old('lifecycle_end_date', $component->lifecycle_end_date))
                            <button type="button" onclick="document.getElementById('lifecycle_end_date').value = ''"
                                    class="text-xs text-gray-400 hover:text-gray-600">Clear</button>
                        @endif
                    </div>
                    <input type="date" id="lifecycle_end_date" name="lifecycle_end_date"
                           value="{{ old('lifecycle_end_date', $component->lifecycle_end_date?->toDateString()) }}"
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2 mb-2" id="selected-tags">
                    @foreach (old('tags', $component->tags->pluck('name')->toArray()) as $tag)
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-300 @enderror">{{ old('description', $component->description) }}</textarea>
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <a href="{{ route('components.show', $component) }}" class="text-sm text-gray-500 hover:text-gray-700">Cancel</a>
            </div>
        </form>
    </div>
