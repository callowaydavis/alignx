...
@section('content')
    <div class="max-w-lg">
        <form name="attributeForm" method="POST" action="{{ route('attributes.update', $attribute) }}" class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <select id="field_type" name="field_type" required
                    ...
                    @foreach ($fieldTypes as $fieldType)
                        <option value="{{ $fieldType->value }}" @selected(old('field_type', $attribute->field_type->value) === $fieldType->value)>{{ $fieldType->label() }}</option>
                    @endforeach
                </select>
                @error('field_type')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
            </div>

            <div id="select-options-container" class="{{ old('field_type', $attribute->field_type->value) !== 'select' ? 'hidden' : '' }}">
                ...
                <textarea id="select-options-input" rows="5"
                          ...
                          placeholder="Option 1&#10;Option 2&#10;Option 3">{{ implode("\n", old('options', $attribute->options ?? [])) }}</textarea>
            </div>
