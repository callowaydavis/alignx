        ...
        <form method="POST" action="{{ route('fact-definitions.update', $factDefinition) }}" class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
            ...
            <div id="select-options-container" class="{{ old('field_type', $factDefinition->field_type->value) !== 'select' ? 'hidden' : '' }}">
                ...
                <textarea id="select-options-input" rows="5"
                          ...
                          placeholder="Option 1&#10;Option 2&#10;Option 3">{{ implode("\n", old('options_text', $factDefinition->options ?? [])) }}</textarea>
            </div>
                    ...
                    @foreach ($types as $type)
                        <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                            <input type="checkbox" name="component_types[]" value="{{ $type->value }}"
                                   @checked(in_array($type->value, old('component_types', $factDefinition->component_types ?? [])))
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Required for Component Types</label>
                <p class="text-xs text-gray-500 mb-3">When checked, this fact must be provided when creating a component of that type.</p>
                <div class="grid grid-cols-2 gap-2">
                    @foreach ($types as $type)
                        <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                            <input type="checkbox" name="required_for_types[]" value="{{ $type->value }}"
                                   @checked(in_array($type->value, old('required_for_types', $factDefinition->required_for_types ?? [])))
                                   class="rounded border-gray-300 text-red-500 focus:ring-red-500">
                            {{ $type->value }}
                        </label>
                    @endforeach
                </div>
                @error('required_for_types')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
            </div>
